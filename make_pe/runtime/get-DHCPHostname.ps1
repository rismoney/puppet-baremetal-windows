...
Function Get-DHCPHostname {
  ...
  $client = New-Object Microsoft.Windows.Internal.DhcpClient
  try {
    [Microsoft.Windows.Internal.DhcpClient]::GetDhcpInterfaces() | % { 
      $val = $client.DhcpRequestParams($_.Id, 12)
      $encode=new-object "System.Text.ASCIIEncoding"
      ($encode.getstring($val)).substring(0,($encode.getstring($val)).length-1)
    }
  }
  finally {
    $client.Dispose()
